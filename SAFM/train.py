...
def train(task_ids, model):
    ...
    if not model:
        model = MODEL_CLASS.from_pretrained('').cuda()
    ...
    if args.multitask_specific:
        for i in range(4):
            TOKENS_WEIGHT = torch.cat((TOKENS_WEIGHT, torch.ones([1]).cuda()))
    if args.distil:
        teacher_model = MODEL_CLASS.from_pretrained('/data//MAC/gpt2').cuda()
    ...
    if not args.fp32:
        ...
        logger.info('optimzer FP16_Optimizer')
