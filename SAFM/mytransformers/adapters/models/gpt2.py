...
class Adapter_NULL(nn.Module):
    ...
    def forward(self, x, residual_input=None):  # , residual_input=None):
        output = x
...
class GPT2AttentionAdaptersModule(BertSelfOutputAdaptersMixin, nn.Module):
    ...
    def add_adapter_NULL(self, adapter_name: str, layer_idx: int):
...
class GPT2DecoderBlockAdaptersMixin(BertEncoderAdaptersMixin):
    ...
    def add_adapter(self, adapter_name: str, layer_idx: int):        ### 被覆盖了
...
class GPT2ModelAdapterMixin(InvertibleAdaptersMixin, ModelAdaptersMixin):
    ...
    def add_adapter(self, adapter_name: str, config=None, typ=None):
        """
        ...
        self._add_adapter(adapter_name, typ=typ)

    # added by cx;
    def get_unique_sorted_adapter_array(self, adapter_functions):
        uni_adapter = np.unique(adapter_functions)   
        adapter_functions_uni = np.array(sorted(uni_adapter.astype(int))).astype(str)
        return adapter_functions_uni
    ...
    def add_adapter_for_mix(self, adapter_name: str, config=None):
        ...
        for i, layer in enumerate(self.base_model.h):
            ...
            c_adapters = self.get_unique_sorted_adapter_array(layer.adapter_function)
    ...
    def train_adapter_for_mix(self, adapter_name):
        ...
        print("gpt2[line136] adapter_setup", adapter_setup)
    ...
    def _add_adapter(self, adapter_name: str, typ=None):
        adapter_config = self.config.adapters.get(adapter_name)
        leave_out = adapter_config.get("leave_out", [])
        for i, layer in enumerate(self.base_model.h):
            if i not in leave_out:
                layer.add_adapter(adapter_name, i, typ=typ)

        self.add_invertible_adapter(adapter_name)    ##########

    def train_adapter(self, adapter_setup: Union[list, AdapterCompositionBlock], skip_layers=None):  # 是啥;
        self.train()
        self.freeze_model(True)
        # print(adapter_setup)
        adapter_setup = parse_composition(adapter_setup)
        print('adapter_setup: {}'.format(adapter_setup))
        self.enable_adapters(adapter_setup, True, False)
        self.enable_invertible_adapters(adapter_setup.flatten())
    ...
    def setup_task_adapter(self, tid, unshared_adapter_layer_list=[]):
        res = []
        for lasyer_id, layer in enumerate(self.base_model.h):
            if lasyer_id in unshared_adapter_layer_list:
                c = layer.setup_task_adapter(lasyer_id, tid, unshared_flag=True) 
            else:
                c = layer.setup_task_adapter(lasyer_id, tid, unshared_flag=False) 
            res.append(c)
        print("gpt result:", res)
        
        ##### !! Attention !!, there is fault in calculating cnt_true;
        cnt_true = 0
        for i in res:
            if i:
                cnt_true += 1
